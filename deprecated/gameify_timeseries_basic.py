...
EGT add-ons:
  - estimate_A_from_series(S, X, v, k, lambda_): fit k×k payoff operator A from series
  - find_ESS(A, ...): enumerate mixed Nash candidates and test ESS via replicator Jacobian

All arrays follow the convention: players × time (N × T) for series.
...
def estimate_A_from_series(S: np.ndarray, X: np.ndarray, v: np.ndarray, k: int, lambda_: float = 0.0):
    """
    ...
    A = MZ_k @ A  # enforce row-centering
...
if __name__ == "__main__":
    ...
    # Build a simple S from rectified PCA as a placeholder (provide your own S for real work)
    U, svals, Vt = np.linalg.svd(X - X.mean(axis=1, keepdims=True), full_matrices=False)
    k = 3
    S = np.abs(U[:, :k])
    v = X  # placeholder payoff field for demo
